...
import '../css/close_ticket.css';
import '../vendor/fontawesome-free/css/all.min.css';
import axios from 'axios';
import { useSpring, animated } from 'react-spring';
import { Modal, Button, Box, Typography, TextField, IconButton,Select,MenuItem ,FormControl ,InputLabel } from '@mui/material';
        ...
        <div className="container-fluid">
          ...
          <div className="card shadow mb-4">
            <div className="card-header py-3">
              <h4 className="m-0 font-weight-bold text-primary">Ticket Filter</h4>
                    ...
                    <tbody>
                      {data.map((item, index) => (
                        <tr key={item.id}>
                          ...
                          <td>{moment(item.Ticket_raise_date).format("DD-MM-YYYY")}</td>
                          <td>{item.Userid}</td>
                          ...
                          <td>
                          <Button variant="text" onClick={() => handleOpen(item)}>ATTEND</Button>
                          </td>
                          <Modal
                            open={open}
                            onClose={handleClose}
                            ...
                            {selectedTicket && (
                              <Fade in={open}>
                                <Box className='close_ticket_modal1'>
                                  ...
                                  <IconButton
                                    style={{ position: 'absolute', top: 5, right: 5, color: 'white' }} onClick={handleClose}
                                    aria-label="Close"
                                  >
                                    <CloseIcon />
                                  </IconButton>
                                  ...
                                  <div className="details">
                                    <div className="section">
                                      ...
                                      <TextField
                                        ...
                                        value={Problem}
                                        variant="outlined"
                                        style={{ margin: '1rem', pointerEvents: "none", cursor: "initial", width: "95%", marginBottom: "2%", marginTop: "1%" }}
                                        InputProps={{
                                          readOnly: true,
                                        }}
                                  ...
                                  <div className="vertical-line"></div>
                                    ...
                                    <div>
                                      ...
                                      <FormControl variant="outlined" style={{ margin: '1rem',width:"28%", }}>
                                            <InputLabel id="Root cause">Root cause</InputLabel>
                                            <Select  label="Root cause"  id="rootCause" value={rootCause}>

                                        {rootCode && rootCode.map((code, index) => (
                                          <option key={index} value={code.Root_cause_id}>
                                    ...
                                    <div className="row1" style={{ display: "flex", justifyContent: "center", marginLeft: "2%" }}>
                                      <label>Status:</label>
                                      <div>
                                        <input type="radio" id="statusOpen" name="status" value="H" checked={status === 'H'} onChange={handleChange} />
                                        <label htmlFor="statusOpen" style={{ marginLeft: 20 }}>Hold</label>
                                        <input type="radio" id="statusClosed" name="status" value="C" checked={status === 'C'} onChange={handleChange} />
                                        <label htmlFor="statusClosed" style={{ marginLeft: 20 }}>Closed</label>
                                      </div>
                                    </div>
