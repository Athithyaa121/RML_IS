...
import { Modal, Typography, Box, IconButton, TextField } from "@mui/material";
...
const Asset_approval = () => {
    ...
    if (searchInput) {
      filteredResults = filteredResults.filter((item) => {
        ...
        return (
          ...
          (item.Asset_type &&
            item.Asset_type.toLowerCase().includes(
              searchInput.toLowerCase()
            )) ||
          (item.Asset_host_name &&
            item.Asset_host_name.toLowerCase().includes(
              searchInput.toLowerCase()
            )) ||
          (item.Asset_date_of_installation &&
            item.Asset_date_of_installation.toLowerCase().includes(
              searchInput.toLowerCase()
            )) ||
          (item.Asset_make &&
            item.Asset_make.toLowerCase().includes(
              searchInput.toLowerCase()
            )) ||
  ...
  const handlesend = () => {
    if (selectedAsset && message) {
      ...
      axios
        .post("http://localhost:3001/email/send_email", { message, plantid })
        .then((response) => {
                ...
                <tbody>
                  {filteredData.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        {moment(item.Asset_date_of_installation).format(
                          "DD-MM-YYYY"
                        )}
                      </td>
